...
const Page: React.FunctionComponent<PageProps> = () => {
    ...
    const deleteMutation = useMutation((id: string) => userMeSaleApi.v1Delete(id), {
        onSuccess: () => {
            toast.success('Xoá thành công');
            router.push(NKRouter.app.userMeSale.index());
        },
    });

    return (
        <div className="relative flex h-full w-full flex-col">
            <Link className="absolute left-4 top-4 z-10" href={NKRouter.app.userMeSale.index()}>
            ...
            <div className="absolute right-4 top-4 z-10 flex gap-2">
                <ModalBuilder
                    ...
                    btnLabel="Xoá"
                    modalTitle={<h2 className="mb-2 text-xl font-medium text-black">Xoá sản phẩm</h2>}
                >
                    {(closeModal) => (
                        <div className="flex w-full flex-col">
                            <p className="mb-4 text-gray-900">Sản phẩm sẽ bị xoá vĩnh viễn sau khi bấm &quot;xoá&quot;</p>
                ...
                <Link href={NKRouter.app.userMeSale.edit(id)}>
                    <button className="z-10 rounded bg-blue-600 px-4 py-1 text-white">Chỉnh sửa</button>
                </Link>
            </div>
            ...
            {userSale.isSuccess ? (
                <>
                    <Swiper className="relative z-0 h-56 w-screen flex-shrink-0  overflow-hidden" slidesPerView={1}>
                        {userSale.data?.imageUrls.map((image) => (
                    ...
                    <div className="relative z-10 flex  h-full w-full flex-col bg-white px-6 shadow-none">
                        ...
                        <p className="text-sm font-semibold text-gray-900">{userSale.data?.productCategory.name}</p>
                        <p className="text-xl font-semibold text-gray-900">{userSale.data?.name}</p>
                        <div className="mt-2 flex w-full justify-between gap-2">
                            <p className="text-2xl font-semibold text-purple-600">{formatMoneyVND(userSale.data?.price ?? 0)}</p>
                        </div>
                                ...
                                <Tab.Panels className={'h-full w-full pb-28  pt-5'}>
                                    <Tab.Panel className={'relative h-full w-full'}>
                                        <div
                                            ...
                                            dangerouslySetInnerHTML={{ __html: userSale.data?.description }}
                                        />
                                    </Tab.Panel>
