            ...
            {userSaleQuery.isSuccess ? (
                <>
                    <div className="fixed bottom-0 left-0 z-30 flex w-full  justify-center gap-4 bg-white px-4 py-10 pt-2 shadow">
                        ...
                        <div className="w-full">
                            <ModalBuilder
                                ...
                                btnProps={{
                                    className:
                                        'flex h-10 w-full items-center justify-center rounded-full bg-purple-600 text-sm font-semibold uppercase text-white',
                                }}
                                btnLabel="Liên hệ"
                                modalTitle={<h2 className="mb-2 text-xl font-medium text-black">Thông tin liên hệ</h2>}
                            >
                                {(closeModal) => (
                                    <div className="flex flex-col gap-2">
                                        <p className="text-base font-medium text-black">
                                            Email: <span className="font-bold">{userSaleQuery.data.createdBy.email}</span>
                                        </p>
                                        <p className="text-base font-medium text-black">
                                            Địa chỉ: <span className="font-bold">{userSaleQuery.data.createdBy.address}</span>
                                        </p>
                                        <p className="text-base font-medium text-black">
                                            Số điện thoại: <span className="font-bold">{userSaleQuery.data.createdBy.phone}</span>
                                        </p>
                                        <button
                                            type="button"
                    ...
                    <Swiper className="relative z-0 h-56 w-screen  overflow-hidden" slidesPerView={1}>
                        {userSaleQuery.data?.imageUrls.map((image) => (
                    ...
                    <div className="relative z-10 flex  h-full w-full flex-col bg-white px-6 shadow-none">
                        ...
                        <p className="text-sm font-semibold text-gray-900">{userSaleQuery.data?.productCategory.name}</p>
                        <p className="text-xl font-semibold text-gray-900">{userSaleQuery.data?.name}</p>
                        <p className="text-base font-bold text-indigo-700">{formatMoneyVND(userSaleQuery.data?.price ?? 0)}</p>
                        ...
                        <div className="">
                            <Tab.Group>
                                <Tab.List className={'flex h-full w-full'}>
                                    <Tab
                                        className={(props) =>
                                    ...
                                    <Tab.Panel className={'relative h-full w-full'}>
                                        <div
                                            ...
                                            dangerouslySetInnerHTML={{
                                                __html: userSaleQuery.data?.description,
                                            }}
                                        />
                                    </Tab.Panel>
